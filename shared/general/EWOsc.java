...
@StudyHeader(
    ...
    menu="MENU_GENERAL",
    overlay=false,
    helpLink="http://www.motivewave.com/studies/elliott_wave_oscillator.htm")
public class EWOsc extends com.motivewave.platform.sdk.study.Study 
{
	enum Values { VAL };
  ...
  public void initialize(Defaults defaults)
  {
    ...
    desc.exportValue(new ValueDescriptor(Values.VAL, get("VAL_EWO"), new String[] {Inputs.INPUT, Inputs.METHOD, Inputs.SLOW_PERIOD, Inputs.FAST_PERIOD}));
  ...
  protected void calculate(int index, DataContext ctx)
  {
    ...
    Color upColor = getSettings().getColor(Inputs.UP_COLOR, ctx.getDefaults().getGreen());
    Color downColor = getSettings().getColor(Inputs.DOWN_COLOR, ctx.getDefaults().getRed());
    
    double diff = fastMA - slowMA;
    series.setDouble(index,  Values.VAL, fastMA - slowMA);
