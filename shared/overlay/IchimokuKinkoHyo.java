...
public class IchimokuKinkoHyo extends Study 
{
  ...
  public void initialize(Defaults defaults)
  {
    ...
    indicators.addRow(new IndicatorDescriptor(TS_IND, get("LBL_TS_IND"), defaults.getOrange(), X11Colors.WHITE, false, false, true));
    indicators.addRow(new IndicatorDescriptor(KS_IND, get("LBL_KS_IND"), defaults.getRed(), X11Colors.WHITE, false, false, true));
    indicators.addRow(new IndicatorDescriptor(CS_IND, get("LBL_CS_IND"), defaults.getBlue(), X11Colors.WHITE, false, false, true));
    indicators.addRow(new IndicatorDescriptor(SSA_IND, get("LBL_SSA_IND"), defaults.getGreen(), X11Colors.WHITE, false, false, true));
    indicators.addRow(new IndicatorDescriptor(SSB_IND, get("LBL_SSB_IND"), defaults.getGreen(), X11Colors.WHITE, false, false, true));

    // Quick Settings (Tool Bar and Popup Editor)
    sd.addQuickSettings(new SliderDescriptor(Inputs.PERIOD, get("LBL_TENKAN_SEN"), 9, 1, 9999, true, () -> Enums.Icon.SINE_WAVE.get()));
    sd.addQuickSettings(new SliderDescriptor(Inputs.PERIOD2, get("LBL_KIJUN_SEN"), 26, 1, 9999, true, () -> Enums.Icon.SINE_WAVE.get()));
    sd.addQuickSettings(new SliderDescriptor(Inputs.PERIOD3, get("LBL_SENKOU_SPANB"), 52, 1, 9999, true, () -> Enums.Icon.SINE_WAVE.get()));
    sd.addQuickSettings(TS_LINE, KS_LINE, CS_LINE, SSA_LINE, SSA_LINE, SSB_LINE, Inputs.TOP_FILL, Inputs.BOTTOM_FILL);
  ...
  protected void calculate(int index, DataContext ctx)
  {
    ...
    var c = new Coordinate(series.getStartTime(index), KS);
